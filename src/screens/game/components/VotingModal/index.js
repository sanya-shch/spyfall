...
function VotingModal({
  ...
  const exhibited = useMemo(
    () => playerData.find((player) => player.uid === exhibitedUid),
    [playerData, exhibitedUid]
  );
  const exhibitor = useMemo(
    () => playerData.find((player) => player.uid === exhibitorUid),
    [playerData, exhibitorUid]
  );
  ...
  const isVotingOver = useMemo(
    () =>
      playerData
        ...
        .every(
          (player) => voteScore?.[player.uid] && voteScore[player.uid] === "spy"
        ),
    [playerData, voteScore, exhibitedUid]
  );
  const isAllPlayerVoting = useMemo(
    () =>
      playerData
        .filter((player) => player.uid !== exhibitedUid)
      ...
      <div className="voting-modal">
        <div className="modal-content">
          ...
          <div className="modal-description">
            <span>{exhibitor?.username}</span> put{" "}
            <span>{exhibited?.username}</span> to the vote
          </div>

          <div className="list-block">
            {playerData.map(
              (player) =>
                player.uid !== exhibitedUid && (
